...
$(document).ready(function(){
    $('li img').on('click',function(){
        ...
        html += img;
        html += '<div>';
        html += '<a class="controls next" href="'+ (index+2) + '"><i class="fa fa-arrow-right"></i></a>';
        html += '<a class="controls previous" href="' + (index) + '"><i class="fa fa-arrow-left"></i></a>';
        html += '</div>';
...
function getBoarders(xml) {
    ...
    var table="<thead class='thead-dark'><tr><th>#</th><th>Name</th><th>Program</th><th>Room</th></tr><thead>";
    var x = xmlDoc.getElementsByTagName("boarders");
    for (i = 0; i <x.length; i++) {
        table += "<tr><td>" +
            (i+1) +
            "</td><td>" +
            x[i].getElementsByTagName("name")[0].childNodes[0].nodeValue.toUpperCase() +
            "</td><td>" +
            x[i].getElementsByTagName("course")[0].childNodes[0].nodeValue.toUpperCase() +
            "</td><td>" +
            x[i].getElementsByTagName("room")[0].childNodes[0].nodeValue.toUpperCase() +
            "</td></tr>";
    }
...
function getNotification(xml) {
    ...
    let msg="<marquee behavior='scroll' scrollamount='3' direction='up' onmouseover='this.stop();' onmouseout='this.start();'> ";
    let x = xmlDoc.getElementsByTagName("notification");
    for (i = 0; i <x.length; i++) {
        msg += "<p>" +
            x[i].getElementsByTagName("massage")[0].childNodes[0].nodeValue.toString() +
